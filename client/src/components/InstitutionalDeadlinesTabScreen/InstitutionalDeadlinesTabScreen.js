...
const InstitutionalDeadlinesTabScreen = ({ value }) => {
  ...
  const formateInternalDeadlines = () => {
    return institutionalDeadlines.map((deadline) => ({
  ...
  const onSaveHandler = async () => {
    let newDeadline = institutionalDeadlines.filter(
      (deadline) => deadline.id === editID
    )[0];

    if (newDeadline) {
      await updateInstitutionalDeadline(editID, newDeadline);
      setEditID(null);
    }
  };
  ...
  const onDeadlineChangeHandler = (e) => {
    ...
    changeInstitutionalDeadline(id, data);
  };
  ...
  const onSubmitHandler = async (e) => {
    ...
    try {
      ...
      const apple = await axios.post(
        `${BASE_URL}/institutionaldeadlines`,
        dataObj,
        axiosConfig
      );

      addToInstitutionalDeadlines(apple.data.newInstitutionalDeadline);
      ...
      <Accordion className={classes.dateListAccordion}>
        ...
        <AccordionDetails className={classes.accordionDetails}>
          ...
          <List className={classes.list}>
            {institutionalDeadlines.map((deadline) => (
                ...
                {editID === deadline.id ? (
                  ...
                  <div className={classes.actionBtns}>
                    ...
                    <div>
                      ...
                      <Button
                        onClick={() => deleteInstitutionalDeadline(deadline.id)}
                        variant="outlined"
